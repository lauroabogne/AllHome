...
class ArchivedGroceryListFragmentViewModel: ViewModel() {
    ...
    suspend fun getArchivedGroceryLists(context: Context):List<GroceryListWithItemCount>{
        groceryLists = AllHomeDatabase.getDatabase(context).groceryListDAO().getDeletedGroceryListsWithItemCount(GroceryListEntityValues.ARCHIVE) as ArrayList<GroceryListWithItemCount>
        return groceryLists
    }
    ...
    suspend fun deleteGroceryList(context: Context,autoGeneratedUniqueId:String,datetime:String,status:Int){
        AllHomeDatabase.getDatabase(context).groceryListDAO().updateGroceryListAsDeleted(autoGeneratedUniqueId,datetime,status, GroceryListEntityValues.NOT_YET_UPLOADED)

    }

    suspend fun restoreArchivedGroceryList(context: Context, autoGeneratedUniqueId:String, datetime:String, status:Int){
        AllHomeDatabase.getDatabase(context).groceryListDAO().updateGroceryListAsActive(autoGeneratedUniqueId,datetime,status, GroceryListEntity.NOT_YET_UPLOADED)
    }

    suspend fun deleteGroceryListPermanently(context: Context,autoGeneratedUniqueId:String,datetime:String,status:Int){
        AllHomeDatabase.getDatabase(context).groceryListDAO().updateGroceryListAsPermanentlyDeleted(autoGeneratedUniqueId,datetime,status, GroceryListEntity.NOT_YET_UPLOADED)
    }
