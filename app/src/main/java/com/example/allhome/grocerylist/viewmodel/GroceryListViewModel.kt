...
class GroceryListViewModel(groceryListEntityParams: GroceryListEntity? = null, groceryItemParams: GroceryItemEntity?) : ViewModel() {
    ...
    suspend fun getGroceryItem(context:Context,itemName:String,unit:String, status:Int):GroceryItemEntity?{
        return AllHomeDatabase.getDatabase(context).groceryItemDAO().getItemByNameAndUnitWithStatus(itemName, unit,status)
