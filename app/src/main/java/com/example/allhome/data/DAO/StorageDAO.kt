...
interface StorageDAO {
    ...
    @Query("UPDATE storage SET name=:name,description=:description,image_name=:imageName,modified=:modified WHERE unique_id=:storageUniqueId AND item_status = 0")
     fun updateStorage(storageUniqueId:String,name:String,description:String,imageName:String,modified:String):Int
    @Query("UPDATE storage SET item_status =:deleteStatus,modified=:currentDateTime WHERE unique_id=:storageUniqueId")
     fun updateStorageAsDeleted(storageUniqueId:String,currentDateTime:String,deleteStatus:Int):Int
    @Query("SELECT * from storage " +
            ...
            " )" +
            " AND item_status = 0")
     fun getStorages(itemName:String,unit:String):List<StorageEntity>
    ...
    @Query("SELECT "+StorageEntity.TABLE_NAME+".*," +
     ...
     fun getStoragesWithItem(itemName:String,unit:String):List<StorageEntityWithStorageItemInformation>
